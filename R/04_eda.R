...
# 5%, 50% e 95% (change the probabilities)
apply(vars, 2, quantile, probs = c(0.05, 0.5, 0.95))


# range function return the min and max for each value
apply(vars, 2, range)
...
my_range <- function(x){
  diff(range(x)) #difference of max and min
}
...
par(mfrow = c(1, 3))
qqnorm(iris$Sepal.Length[iris$Species == "setosa"], #quantile plot
       main = "setosa")
