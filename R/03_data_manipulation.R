...
# Let's apply the `head()`, `dim()` and `summary()` functions to inspect all files.
#Try to understand based on the output and the help page (e.g.: `?head
#what each of the functions returns.
...
# Understanding the object `splist` (species list)
head(splist)
dim(splist)
summary(splist)
...
#by= specify the common column
#default is inner_join
...
select_test <- select(comm_total, TaxCode, Abundance, Sites)

select_test2 <- comm_total[,c("TaxCode","Abundance","Sites")]
